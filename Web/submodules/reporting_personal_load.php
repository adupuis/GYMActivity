...
foreach( $geny_ar->getActivityReportsListWithRestrictions( array( "profile_id=".$profile->id,"activity_report_status_id != $ars_p_user_approval_id", "activity_report_status_id != $ars_refused_id", "activity_report_status_id != $ars_removed_id" ) ) as $ar ){
...
// Création des données de reporting pour la charge par tâche
$load_by_tasks = array();
$tmp_task = new GenyTask();
// $reporting_data_tasks[$ar->profile_id][$geny_activity->assignement_id][$geny_activity->task_id] += $geny_activity->load;
foreach( $reporting_data_tasks as $profile_id => $data ){
	foreach( $data as $assignement_id => $task_data ){
		$ttl=0;
		foreach( $task_data as $task_id => $total_load ){
			$tmp_task->loadTaskById($task_id);
			$ttl += $total_load;
			error_log("\$ttl=$ttl \$task_id=$task_id task name=".$tmp_task->name,0);
		}
		$load_by_tasks[$tmp_task->name]=$ttl/8;
	}
}
$load_by_tasks_js_data = "";
$tmp_array=array();
foreach( $load_by_tasks as $task_name => $load ){
	$tmp_array[]= "['$task_name', $load]";
}
$load_by_tasks_js_data = implode(",",$tmp_array);
...
<script type="text/javascript">
	...
	function drawChart() {
		...
		data2.addColumn('string', 'Tâche');
		data2.addColumn('number', 'Charge');
		data2.addRows([
		<?php echo $load_by_tasks_js_data;?>
		]);

		// Set chart options
		var options = {'title':'Reporting de charge - charge/tâche - Entre <?php echo "$start_date" ?> et <?php echo "$end_date" ?>',
				'is3D': true,
				'width':800,
				'height':300};
...
<div id="mainarea">
	...
	<p class="mainarea_content">
		...
		<form id="formID" action="loader.php?module=reporting_personal_load" method="post">
			<p>
