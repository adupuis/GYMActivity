...
foreach( $geny_intranet_type->getAllIntranetTypes() as $tmp ) {
	...
	$edit = "<a href=\"loader.php?module=intranet_type_edit&load_intranet_type=true&intranet_type_id=$tmp->id\" title=\"Editer le type de catégorie Intranet\"><img src=\"images/$web_config->theme/conges_admin_edit_small.png\" alt=\"Editer le type de catégorie Intranet\"></a>";

	$remove = "<a href=\"loader.php?module=intranet_type_remove&intranet_type_id=$tmp->id\" title=\"Supprimer définitivement le type de catégorie Intranet\"><img src=\"images/$web_config->theme/conges_admin_remove_small.png\" alt=\"Supprimer définitiement le type de catégorie Intranet\"></a>";
...
<div id="mainarea">
	...
	<p class="mainarea_content">
		...
		<script>
			...
			<?php
				if( array_key_exists( "GYMActivity_intranet_type_list_php", $_COOKIE ) ) {
					$cookie = json_decode( $_COOKIE["GYMActivity_intranet_type_list_php"] );
				}
				...
				<table id="intranet_type_list_table" style="color: black; width: 100%;">
					...
					<?php
						foreach( $data_array as $da ){
							echo "<tr><td>".$da[1]."</td><td>".$da[2]."</td><td>".$da[3]."</td><td>".$da[4]."</td><td><center>".$da[5]."</center></td></tr>";
						}
					?>
					</tbody>
					<tfoot>
