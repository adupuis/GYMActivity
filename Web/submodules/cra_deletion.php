...
if(isset($_POST['cra_action']) && ($_POST['cra_action'] == "cra_deletion") ){
	if( $_POST['cra_action'] == "cra_deletion" ){
		if( isset( $_POST['activity_report_id'] ) ){
			...
			if($ok_count > 0 ){
				...
				$notif->insertNewGroupNotification(1,"$screen_name viens de demander la suppression de $ok_count rapport(s) d'activité, merci de faire le nécessaire.");
				// Notification des superusers
				$notif->insertNewGroupNotification(2,"$screen_name viens de demander la suppression de $ok_count rapport(s) d'activité, merci de faire le nécessaire.");
				if($ok_count == 1)
					$gritter_notifications[] = array('status'=>'success', 'title' => 'Succès','msg'=>"$ok_count rapport est désormais en attente de suppression par le management.");
				else
					$gritter_notifications[] = array('status'=>'success', 'title' => 'Succès','msg'=>"$ok_count rapports sont désormais en attente de suppression par le management.");
			}
		}
	}
}
...
<div id="mainarea">
	...
	<p class="mainarea_content">
		...
		Ce formulaire permet de demander la suppression de rapports d'activités déjà validés.<br />
		<strong class="important_note">Important :</strong> Ce formulaire contient tous les rapports validés par le management.<br />
		</p>
		<script>
			(function($) {
			/*
		...
		<form id="formID" action="loader.php?module=cra_deletion" method="post" class="table_container">
...
<?php
	$bottomdock_items = array('backend/widgets/cra_add.dock.widget.php','backend/widgets/cra_list.dock.widget.php','backend/widgets/cra_validation.dock.widget.php');
?>
